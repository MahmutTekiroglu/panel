...
function TelegramUsers(){
    ...
    const [userForm, setUserForm] = useState({name:"", code: "", password:"", role:"", status:"1", id: ""})
    const [formType, setFormType] = useState("")
    const [onSweetalert, setOnSweetalert] = useState({isActive: false, type: "info", title: "Telegram Kullanıcı Silinecek", message: "Seçtiğiniz kullaniciyı silmek istediğnize emin misiniz", confirmText: "Evet, Sil", cancelText:"Vazgeç"})
    ...
    async function UserList() {
        SetLoading(true)
        let data = await WebServiceRequest(userFilter, "telegramUsers/list")
        await setUserList(data)
        SetLoading(false)
    }

    function userAdd() {
        setUserForm({name:"", code: "", password:"", role:"", status:"1", id: ""})
        setModal(true); 
        setFormType("add")
    }

    async function userUpdate(data) {
        setUserForm({code:data.code, name:data.name, password:data.password, role:data.role, status:data.status, id: data.id})
        setFormType("update")
        setModal(true)
    }

    async function UserSave() {
        let data = await WebServiceRequest(userForm, `telegramUsers/${formType}`)
        if (data[0].S == "T") {
    ...
    async function userDelete(id) {
        let data = await WebServiceRequest({id:id}, `telegramUsers/delete`)
        if (data[0].S == "T") {
            Toast("success", data[0].message)
            UserList()
        }else {
    ...
    return (
        <React.Fragment>
            ...
            <div className="page-content">
                ...
                { userList.length == 0 && !loading && <NoItems text={"Telegram Kullanıcı Kaydı Bulunamadı"} func={userAdd}/> }
                { loading && <ListLoader text={"Telegram Kullanıcı kayıtları listeleniyor..."}/> }
                            ...
                            <div className="table-responsive">
                                ...
                                <thead>
                                    <tr>
                                        ...
                                        <th className="text-center" style={{width:"15%"}}>Kod</th>
                                        <th className="text-center" style={{width:"15%"}}>Durum</th>
                ...
                <Modal isOpen={modal} centered={true}>
                    ...
                    <div className="modal-body">
                        <form onSubmit={(e) => {UserSave(); e.preventDefault();}}>
                            ...
                            <div className="mb-3">
                                <label className="form-label" htmlFor="name">Telegram Kodu</label>
                                <input
                                    type="text"
                                    className="form-control"
                                    placeholder="Telegram Kodu"
                                    value={userForm.code}
                                    required
